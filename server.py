...
def thought_details(thought_id):
    ...
    mysql = connectToMySQL("dojo_thoughts")
    query = "SELECT users.first_name, users.last_name FROM liked_thoughts JOIN users on liked_thoughts.users_id_users = users.id_users WHERE thoughts_id_thoughts = %(t_id)s ORDER BY users.first_name;"
    data = {
            't_id': thought_id
    }
